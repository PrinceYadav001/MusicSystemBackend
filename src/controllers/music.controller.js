...
async function createMusic(req , res){
    ...
    res.status(201).json({
        message:"Music created successfull",
        music:{
            id:music._id,
            url:music.uri,
            title:music.title,
...
async function getAllMusics(req , res){
    const musics = await  musicModel.find().skip(3).limit(1).populate("artist" , "username email");
...
async function getAllAlbum(req, res){
    ...
    res.status(200).json({
        message:"Album fetch successfull",
        albums: albums
    })
...
async function getAllAlbumByID(req ,res){
     ...
     res.status(200).json({
        message:"Musics are fetch successfully",
        album:album
     })
}
